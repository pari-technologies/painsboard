...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if($input_new_pass != $input_re_pass){
    ...
    echo "</script>";
      ...
      $sql = "UPDATE users set password = '".$input_new_pass."' where id = '".$input_id."'";

      if ($link->query($sql) === TRUE) {
                         
        echo "<script>";
        echo "alert('Reset password success!');";
        echo "window.location = ' ../org/signin.php?id=".$input_org_id."';"; // redirect with javascript, after page loads
        echo "</script>";
