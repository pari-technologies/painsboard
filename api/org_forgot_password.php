...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  $input_username = $_POST["email"];
   ...
   // Attempt select query execution
                      $sql = "SELECT * FROM organization where org_email = '".$input_username."'";
                      ...
                      if($result = mysqli_query($link, $sql)){
                          if(mysqli_num_rows($result) > 0){
                            while( $row = mysqli_fetch_assoc($result)){
                              ...
                              $subject = "PainsBoard - Reset Organization Password";

                              $message = '<html><body>';
                              $message .= '<h3>Hello!</h3>';
                              $message .= '<p>Please click this <a href="https://painsboard.com/reset_password.php?id='.$new_array[0]['id'].'&token='.generateRandomString().'" style="color: blue;">link</a> to reset your password.</p>';
                              $message .= '</body></html>';
